...
public:
  ...
  // translate : Shape double double -> Shape[same kind]
  // Purpose: move the shape by some amount
  virtual Shape* translate ( double dx, double dy ) = 0;

  // show : Shape -> int
  // Purpose: to show the shape by printing it
  virtual int show ( ) = 0;
};
...
public:
  ...
  // translate : Circle double double -> Shape
  // Purpose: move the shape by some amount
  Shape* translate ( double dx, double dy ) {
    // Template: this, this->center, this->radius, dx, dy
      
    // Example 1
    // this->center = new Point(0.0, 0.0)
    //return (new Circle( new Point (10.0, 10.0), 25.0 ));

    // These were a bad path...
    //return (new Circle( new Point (10.0, 10.0), this->radius ));
    //return (new Circle( new Point (0.0+10.0, 0.0+10.0), this->radius ));
    //return (new Circle( new Point (this->center->x+10.0, 0.0+10.0), this->radius ));
    
    // Generalize 1
    //return (new Circle( new Point (10.0, 10.0), this->radius ));
    //return (new Circle( SomeAwesomeFunction( this->center ), this->radius ));
...
public:
  ...
  // translate : Square double double -> Shape
  // Purpose: move the shape by some amount
  Shape* translate ( double dx, double dy ) {
    // Template: this, this->top_left, this->side_len, dx, dy    
    return new Square( (this->top_left)->translate(dx, dy), this->side_len );
  }
};
...
// hybercubate : Shape -> Shape
// Purpose : hybercubate the shape (c.f. Wikipedia)
Shape* hybercubate ( Shape* s ) {
  // Template: s, s->area(), s->translate(dx,dy), s->show()
  
  return s->translate(s->area(),s->area()) ;
}
...
int main () {
  ...
  printf ( "The answer is %f\n",
           grandpa->side_len );
  // gopherTheShape is a Shape and it don't have a radius
  //printf ( "The answer is %f\n",
  //         gopherTheShape->radius );

  /*
1.  area(gopherTheShape)
  
2.  [ s = gopherTheShape] 
  return s->area();
  
3. 
  return gopherTheShape->area();

4. 
  return (new Circle( new Point( 0, 0 ), 25 ))->area();

  5.
  [this = (new Circle( new Point( 0, 0 ), 25 ))] 
  return M_PI * this->radius * this->radius ;
  
  6. 
  return M_PI * (new Circle( new Point( 0, 0 ), 25 ))->radius * this->radius ;

  7. 
  return M_PI * 25 * this->radius ;

  8. 
  return M_PI * 25 * (new Circle( new Point( 0, 0 ), 25 )))->radius ;
...
*/
  ...
  grandpaTheShape->translate(10, 10)->show();   
  printf ( "\n, but should be\n  Square at (%f,%f) with %f sidelen\n",
          87.0, 109.0, 12.0) ;
  
  printf ( "The answer is \n  " );
  gopherTheShape->translate(10, 10)->show();
  printf ( "\n, but should be\n  Circle at (%f,%f) with %f radius\n",
          10.0, 10.0, 25.0) ;

  printf ( "The answer is \n  " );
  hybercubate(grandpaTheShape)->show();   
  printf ( "\n, but should be\n  Square at (%f,%f) with %f sidelen\n",
          77.0+144.0, 99.0+144.0, 12.0) ;
  printf ( "The answer is \n  " );
  hybercubate(gopherTheShape)->show();
  printf ( "\n, but should be\n  Circle at (%f,%f) with %f radius\n",
          1962.5, 1962.5, 25.0) ;
...
// DONE square
// DONE area
// DONE substitution on area
// DONE translate
// XXX hybercubate
// DONE good tests: all possibilities
// XXX haiku
