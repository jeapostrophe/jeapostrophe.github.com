...
;; strict : AEV -> AEV (but not a promiseV)
(define (strict maybe-v)
  (type-case
   AEV maybe-v
   [numV (n) maybe-v]
   [closureV (name body env) maybe-v]
   [promiseV (expr env maybe-the-answer)
             (if maybe-the-answer
                 maybe-the-answer
                 (local [(define the-right-stuff (strict (interp* expr env)))]
                        (set-promiseV-ans! maybe-v the-right-stuff)
                        the-right-stuff))]))
...
(test (interp (parse '(with (x (+ 5 (fun (y) y)))
                            12)))
      ;; Lazy
      12
      ;; Eager
...
(test (interp (parse '(with (f (fun (x) (+ x x))) (f 5))))
      ;; Lazy
      10
      ;; Eager
      ;10
      )
(test (interp (parse '(with (x (+ 5 5)) (+ x x))))
      ;; Lazy (do it once)
      20
      ;; Eager (does it once)
      ;20
