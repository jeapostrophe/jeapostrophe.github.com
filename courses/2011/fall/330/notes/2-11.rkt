...
;; lookup-binding : symbol? DefrdSubst -> number
(define (lookup-binding s ds)
  (type-case
   DefrdSubst ds
   [mtSub ()
          (error 'interp "Unbound identifier ~e" s)]
   [aSub (name named-thing the-others)
         (if (symbol=? name s)
             named-thing
             (lookup-binding s the-others))]))

;; interp : AE (listof FunDef) DefrdSubst -> meaning
        ...
        (local [(define fun-def (lookup-fundef fun-name fun-defs))]
               (interp* (fundef-body fun-def)
                        ...
                        (aSub (fundef-arg-name fun-def)
                              ...
                              ;; Evil = Dynamic Scope
                              ;ds
                              ;; Saintly = Static Scope
                              (mtSub))))]))

(define (interp some-ae fun-defs)
  (interp* some-ae fun-defs (mtSub)))
