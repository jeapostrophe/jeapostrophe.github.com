  ...
  (type-case
   ...
   [binop
    ...
    (numV
     (lift-numV
      op
      ;; strictness points --- places where we need to force promises
      (! (calc lhs $))
      (! (calc rhs $))))]))

;; ! : AEV -> AEV (except not exprV)
(define (! v)
  (type-case
   AEV v
   [exprV
    ...
    (cond
      [maybe-ans
       maybe-ans]
      [else
       (define ans (! (calc expr env)))
       ;; store the ans...?
       ;; (exprV expr env ans)
       (set-exprV-ans! v ans)
       ;; we don't need these anymore:
       ;; (set-exprV-env! v (mtEnv))
       ;; (set-exprV-expr! v (num 0))
       ans])]
   [else
    v]))
