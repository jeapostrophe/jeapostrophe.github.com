  ...
  (type-case
   ...
   [fun
    ...
    (let ()
      ...
      (fun new-arg-id (alpha-vary/env (hash-set env arg-id new-arg-id) body)))]
   [app
    (fun-expr arg-expr)
    (app
...
(define (generate-constraints e-id e)
  (type-case
   ...
   [tfirst
    (e)
    (begin
      (define first-id (gensym 'tfirst-e))
      (append
       (list (eqc (t-var first-id)
                  (t-list (t-var e-id))))
       (generate-constraints first-id e)))]
   [trest
    (e)
    (begin
      (append
    ...
    [else
     ...
     (cond
       ...
       [(t-var? left)
        ...
        (unify/loop (subst-cs left right cs)
...
(define (occurs? maybe-inside t)
  (or (equal? maybe-inside t)
      (and (t-list? t)
           (occurs? maybe-inside (t-list-elem t)))
      (and (t-fun? t)
           (or (occurs? maybe-inside (t-fun-arg t))
               (occurs? maybe-inside (t-fun-result t))))))
...
(infer-type (parse '(rec (f (fun (x) (f x))) f)))
(infer-type (parse '(rec (b b) (fun (a) b))))

;; (infer-type (parse '(fun (x) (x x))))
;; (infer-type (parse '(rec (b (tcons b b)) b)))
