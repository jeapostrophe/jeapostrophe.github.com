  ...
  (type-case
   ...
   [binC
    ...
    (interp lhs env
            (λ (lhs-v)
              (interp rhs env
                      ;; Closure: Code Ptr, 3 Local (return, op,
                      ;; lhs-v)

                      ;; This continuation is 4 pointers
                      (λ (rhs-v)
                        (return
                         (numV
                          (op (numV-n lhs-v)
...
;;  PUSH 92
;; 92 return x
;;  REMOVE that 5 and put in the ACCUM

;; XXX Final version of add-two-numbers.com

(define (read-number/and-then/auto prompt)
  (let/cc the-continuation
    (read-number/and-then prompt the-continuation)))

(define (add-two-numbers.exe.com/auto)
  (displayln
   (+ (read-number/and-then/auto "First")
      (read-number/and-then/auto "Second"))))

;; (add-two-numbers.exe.com/auto)

(define (add-many-numbers.exe.com/auto)
  (letrec ([ask-until-zero
            (λ (ignored)
              (let ([x (read-number/and-then/auto
                        "Gimme another")])
                (+ x
                   (if (zero? x)
                     0
...
;; (add-many-numbers.exe.com/auto)

;; XXX Talk about continuation marking
...
(define (!/k+ n k+)
  (if (zero? n)
    (begin
      (printf "the k+ info was: ~a\n" (first k+))
      ((second k+) 1))
    (!/k+ (sub1 n)
          (list
           (cons (list '!/k+ n) (first k+))
           (λ (sub)
             (printf "!/k+ mult ~a ~a\n" n sub)
             ((second k+) (* n sub)))))))
...
(test (!/tr+ 4 1) (* 4 3 2 1))
(test (!+ 4) (* 4 3 2 1))
(test (!/k 4 (λ (x) x)) (* 4 3 2 1))
(test (!/k+ 4 (list empty (λ (x) x))) (* 4 3 2 1))
...
(define the-threads empty)

(define (after-you-francois)
  (let/cc current-thread
    ;; take the current thread and put it on the-threads
    (set! the-threads (snoc the-threads current-thread))
    ;; then remove the top of the-threads and run it
    (define next-thread (first the-threads))
    (set! the-threads (rest the-threads))
    (next-thread)))

(define (scheduler init-threads)
  (printf "Ready to go\n")
  (set! the-threads (rest init-threads))
  ((first init-threads)))
...
;; XXX Ambiguous choice

(define (fail)
  (error 'fail))

(define (choose-a-number l)
  (let/cc return
    (define og-fail fail)
    (for ([e (in-list (shuffle l))])
      (let/cc the-next-choice
        (set! fail the-next-choice)
        (return e)))
    (og-fail)))

(let ([x (choose-a-number (list 10 3 4 6 8 2 1))]
      [y (choose-a-number (list 10 3 4 6 8 2 1 2))])
  (unless (= (+ x y) 10)
    (fail))
  (printf "~a and ~a\n"
          x y)
