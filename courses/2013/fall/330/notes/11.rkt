...
(define (parse se)
  (cond
    ...
    [(and (s-exp-list? se) (= (length (s-exp->list se)) 4))
     (fdC (s-exp->symbol (second (s-exp->list se))) (s-exp->symbol (third (s-exp->list se))) (parse (fourth (s-exp->list se))))]
    [else
     (begin
     (display se)
     (error 'parse "You are wrong"))]))

(test (parse '(fun foo x (+ x 1)))
      (fdC 'foo 'x (addC (idC 'x) (numC 1))))
(test (parse '((fun double x (+ x x)) 13))
      (appC (fdC 'double 'x (addC (idC 'x) (idC 'x))) (numC 13)))
...
(define (interp p env)
  (type-case
    ...
    [appC
     (fun arg)
       (interp (fdC-body fun) (extend-env 
                                  (bind (fdC-arg fun) (interp arg env))
                                  mt-env))]
    [fdC
     (name arg body)
     (funV name arg body)]
...
(test (interp* '(* 23 5))
      (numV 115))
(test (interp* '((fun const5 x 5) 13))
      (interp* '5))
(test (interp* '((fun double x (+ x x)) 13))
      (interp* '(+ 13 13)))
