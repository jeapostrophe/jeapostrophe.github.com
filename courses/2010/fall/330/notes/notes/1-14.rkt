...
(define-type CFWAE-Value
  ...
  [closureV (param symbol?)
            (body CFWAE?)
            (env Env?)]
  [exprV (delayed-expr CFWAE?)
         (original-env Env?)
         (cache (box/c (or/c #f CFWAE-Value?)))])
...
; delay : CFWAE Env -> exprV
(define (delay e env)
  (exprV e env (box #f)))

; strict : CFWAE-Value -> CFWAE-Value (except one that isn't an exprV)
(define (strict cv)
  (type-case CFWAE-Value cv
    [exprV (arg-expr env cache)
           (if (unbox cache)
               (unbox cache)
               (begin
                 ; Uncomment this to see that we are lazy
                 #;(printf "Evaluating ~S\n" arg-expr)
                 (local [(define actual-v (strict (interp arg-expr env)))]
