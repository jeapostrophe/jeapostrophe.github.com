...
(define (closureV param body env)
  (λ (arg-value k-at-call-time)
    (interp/k body (anEnv param arg-value env) k-at-call-time)))
...
(define (interp/k e env k)
  (type-case RCFWAE e
    ...
    [withcc (k-id body-expr)
      (local [; Save the continuation
              (define the-continuation
                (λ (arg-v k-at-call-time)
                  ; k-at-call-time is ignored because continuations
                  ; forget their calling context
                  (k arg-v)))
              ; Map it to the id
              (define new-env
                (anEnv k-id the-continuation
                       env))]
        ; Interp the body, in that new enviroment
        (interp/k body-expr
                  new-env
                  k))]))
...
; No problems: 
; withcc problems: 1 ; <--- 2nd place!
; Other problems: 5 ;<--- winner!
; 42: 1
